const mediaAttributes = ['slug', 'description', 'source_url', 'duration', 'type']
const teamMemberAttributes = ['slug', 'full_name', 'role', 'status', 'email', 'bio']
const ventureAttributes = ['slug', 'name', 'overview', 'jurisdiction', 'vertical', 'industry', 'status',
  'phone', 'email', 'logo_url', 'website_url']

function present (item, type = 'venture', mediaToPlay = {}) {
  if (type !== 'venture') throw new Error(`no presenter written for type "${type}"`)

  if (item.map) {
    return item.map(i => { return formatVenture(i, mediaToPlay) })
  } else {
...
function formatVenture (venture, mediaToPlay) {
  ...
  if (venture.tags) {
    forResponse.tags = venture.tags.map(tag => { return { name: tag.name } })
  }

  if (venture.media) {
    forResponse.media = venture.media.map(mediaItem => { return formatMediaItem(mediaItem) })
  }

  if (venture.teamMembers) {
    forResponse.team_members = venture.teamMembers.map(teamMember => { return presentTeamMember(teamMember, venture.slug) })
  }
...
function formatMediaItem (mediaItem) {
  ...
  mediaAttributes.forEach(attrName => { formatted[attrName] = mediaItem[attrName] })
...
function presentTeamMember (teamMember, ventureSlug) {
  ...
  teamMemberAttributes.forEach(attrName => { formatted[attrName] = teamMember[attrName] })
...
function presentTeamMembers (teamMembers, ventureSlug) {
  return teamMembers.map(tm => { return presentTeamMember(tm, ventureSlug) })
}

...
class BaseController {
  constructor (name, options = {}) {
    ...
    this.pluralizedModel = options.pluralizedModel || `${this.singularizedModel}s`
  ...
  index (req, res, next) {
    ...
    this.model.findAll({ limit: limit,
  ...
  update (req, res, next) {
    this.model.update(req.body, { where: { [this.instanceSearchAttribute]: req.params.id } })
    .then(result => {
  ...
  delete (req, res, next) {
    this.model.destroy({ where: { [this.instanceSearchAttribute]: req.params.id } })
    .then(result => {
      res.send({ [this.singularizedModel]: result })
    }).catch(err => {
      next(err)
